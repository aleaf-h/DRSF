...
class ExpMultiGpuTrainer(AbstractTrainer):
    ...
    def _train_settings(self, model_cfg, data_cfg, config_cfg):
        ...
        self.train_loader = data.DataLoader(self.train_set, shuffle=False,
                                            ...
                                            batch_size=data_cfg["train_batch_size"],drop_last=True)
        ...
        if not self.debug:
            ...
            self.dir = "/home/jiao/users/gaowanhui/run/" + self.model_name + "/" + self.run_id
        ...
        self.recons_loss_meter2 = AverageMeter()
    ...
    def _save_ckpt_auc(self, step, best=False):
        save_dir = os.path.join(self.dir, f"best_model_auc{step}.bin")
        torch.save({
            "step": step,
            "best_step": step,
            "best_metric": self.best_metric,
            ...
            for epoch_idx in range(start_epoch, self.num_epoch + 1):
                ...
                for batch_idx, train_data in train_generator:
                    ...
                    with autocast():
                        ...
                        # print(len(self.model.module.loss_inputs['recons']))
                        recons_loss = exp_recons_loss(self.model.module.loss_inputs['recons'], (in_I, Y))
                        recons_loss2 = exp_recons_loss(self.model.module.loss_inputs['recons2'], (in_I, Y))
                        contra_loss = self.contra_loss(self.model.module.loss_inputs['contra'], Y)

                        loss += self.lambda_1 * recons_loss + 0.5 * recons_loss2 + self.lambda_2 * contra_loss
                    ...
                    self.recons_loss_meter2.update(reduce_tensor(recons_loss2).item())
    ...
    def validate(self, epoch, step, timer, writer):
        ...
        with torch.no_grad():
            ...
            if self.eval_metric == 'Acc' and cur_acc >= self.best_metric:
                ...
                self.best_step = step
                self._save_ckpt(step, best=True)
            if cur_auc >= 0.9200 and cur_acc>=0.8800:#if cur_auc >= 0.9880 and cur_acc>=0.9600:
                # c40 9200/8800#F2F  9650/9140  #DF 9900/9400 #NT  8050/8850  #FS 9850/9400  #wild  9000/8250  #celeb 9990/9900
                self._save_ckpt_auc(step, best=True)
