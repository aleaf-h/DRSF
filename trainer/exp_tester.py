...
class ExpTester(AbstractTrainer):
    ...
    def _test_settings(self, model_cfg=None, data_cfg=None, config_cfg=None):
        ...
        self.test_loader = data.DataLoader(self.test_set, shuffle=False,
                                           batch_size=self.test_batch_size,drop_last=True)
    ...
    def test(self, display_images=False):
        ...
        for idx, test_data in test_generator:
            ...
            image_tensor =self.model.loss_inputs['recons']
            # print("ee",image_tensor.shape)
            # for i in image_tensor:
            #     print("ee", i.shape)
            #     # 提取指定通道
            #     image_channel = i[1, 1, :, :]
            #     image_channel = image_channel.detach().cpu().numpy()
            #     # 绘制热图
            #     plt.imshow(image_channel, cmap='hot', interpolation='nearest')
            #     plt.colorbar()  # 显示色条
            #     plt.title(f'Heatmap of Channel {0}')
            #     plt.show()

            # if display_images: #and idx == 1: #t_idx:
            #     sample_recons = list()
            #     images = I  # [:4]
            #     diff = list()
            #     #print(images)
            #     for _ in self.model.loss_inputs['recons']:
            #         sample_recons.append(_.to("cpu"))
                    #print(type(_.to("cpu")))
                    # diff.append(10-images + _.to("cpu"))
                # show images
                # print("111",images)
                # print("222",sample_recons)
                ...
                # pred = Y_pre    #[:4]
                # gt = Y#[:4]
                # self.plot_figure(images, pred, gt, 2, categories, show=True)
